...
(function bhMenu() {
 ...
 menuCloser.addEventListener('click', closeMenu); // Shaded area close
 ...
 let measureInterval; // Without layout thrashing

 function measureMenus() {
 console.log('measureMenus');
 console.timeStamp('measureMenus'); // Has CSS arrived yet? body bg att should be fixed if so

 if (window.getComputedStyle(document.querySelector('body')).getPropertyValue('background-attachment') === 'scroll') {
 ...
 const subs = document.querySelectorAll('.menu-item-has-children ul'); // Write cycle
...
(function alkotasokTable() {
 ...
 document.querySelector('table').classList.add('alkotasok'); // Make actual header main rows span 3 columns
