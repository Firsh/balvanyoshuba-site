...
(function kereso() {
 ...
 helper.clearRefinements(); // Clear hits
 ...
 const searchPayload = location.search.replace(/\/$/gim, '');
 /*.substr(1)*/

 const searchBits = searchPayload.split('/'); // If last one is digits
 ...
 if (typeof item._highlightResult.time.matchedWords !== 'undefined' && item._highlightResult.time.matchedWords.length > 0) {
 time = `
 <div>
 ...
 <span>${item.breadcrumb ? instantsearch.highlight({
 attribute: 'breadcrumb',
 hit: item
 }) : item.category}</span>
 </div>`.replace(/»/g, '<span class="bc-separator">»</span>');
 const snippet = instantsearch.snippet({
 attribute: 'content',
 hit: item
 }).replace(/___/gi, ' ');
 return `
 ...
 const currentRefinement = renderOptions.currentRefinement,
 ...
 const midSize = 2;
 let posOfCurrentInPages; // Keep only 2 on the LEFT side of the current one 1 ... 3 4 [5]
 ...
 buttons.pages.push(`<a class="page-numbers" data-value="${page}" href="${createURL(page)}">${page + 1}</a>`);
 }
 }
 } catch (err) {
 _iterator2.e(err);
 ...
 const hitsPerPage = renderOptions.hitsPerPage,
 nbHits = renderOptions.nbHits,
 ...
 // Rendering logic
 const items = renderOptions.items,
 widgetParams = renderOptions.widgetParams;
 const container = widgetParams.container;
 ...
 if (filteredCount !== 5) {
 updateTitle({
 facet: `a ${filteredCount} kategóriában`
 });
 } else {
 updateTitle({
 facet: `az ${filteredCount} kategóriában`
 });
 }
 }
 ...
 const container = widgetParams.container;
 const customLimit = widgetParams.customLimit; // Reorder items to ensure refined ones always show up
 ...
 items = refined.concat(notRefined.slice(0, customLimit - refined.length)).sort((a, b) => b.count - a.count); // If there is a discrepancy beween the composition of items vs. the dislayed
 ...
 let reRender = items.length === 0 || items.length !== labelCount || !items.every(item => {
 ...
 <a href="${createURL(item.value)}" class="${item.isRefined ? 'cat-item cat-selected' : 'cat-item'}" data-value="${item.value}">
 ...
 limit: 25,
 customLimit: 10
 }), customStats(), instantsearch.widgets.poweredBy({
 ...
 container.innerHTML = '<div id="jig1" class="justified-image-grid jig-preset-c2"><div class="jig-clearfix"></div></div>'; // Showing deferred results with JIG
 ...
 item = itemsForJIG[i]; // Medium is not created if not tall enough, full is always there I hope :D

 item.usableURL = item.medium ? item.medium : item.full; // Without mediumW, the fullW is not scaled proportionally to match expected thumb height
 ...
 item.description = `A kép itt szerepel: ${contextDisplay}`; //imageID = item.objectID.split('#').pop(); // due to RML shortcuts, IDs aren't unique :D ...
 ...
 itemsForJIG[previousEncounter].allContexts = [`<a href="${itemsForJIG[previousEncounter].context.url}">${itemsForJIG[previousEncounter].context.title}</a> (${itemsForJIG[previousEncounter].context.category})`];
 } // Push the current hit, maybe there will be more


 itemsForJIG[previousEncounter].allContexts.push(contextDisplay);
 hitCount = itemsForJIG[previousEncounter].allContexts.length; // Update the displayed hit's desc with all subsequent hits

 itemsForJIG[previousEncounter].description = `A kép ${hitCount} helyen is megtalálható az oldalon:<br>` + itemsForJIG[previousEncounter].allContexts.join(',<br>'); // Indicate in the caption that it's a multi-result

 itemsForJIG[previousEncounter].caption = `${hitCount} helyen szerepel!`; // This prevents duplicates from showing up
 ...
 var _iterator3 = _createForOfIteratorHelper(itemsForJIG),
 _step3;
 ...
 if (item._snippetResult && item._snippetResult.content && item._snippetResult.content.matchLevel !== 'none') {
 ...
 Object.assign(titleData, newData); // Craft the h1 tag value
 ...
 titleData.heading.innerHTML = newTitle; // Now doc/window title
